...
static void
render_pose_keypoints(Mat& frame,
                      ...
                      float scale)
{
  ...
  unsigned int pairs[] = { 1, 2, 1, 5,  2,  3,  3, 4,  5,  6,  6,  7,
                           1, 8, 8, 9,  9,  10, 1, 11, 11, 12, 12, 13,
                           1, 0, 0, 14, 14, 16, 0, 15, 15, 17 };
  float colors[] = { 255.f, 0.f,   85.f,  255.f, 0.f,   0.f,   255.f, 85.f,
                     0.f,   255.f, 170.f, 0.f,   255.f, 255.f, 0.f,   170.f,
                     255.f, 0.f,   85.f,  255.f, 0.f,   0.f,   255.f, 0.f,
                     0.f,   255.f, 85.f,  0.f,   255.f, 170.f, 0.f,   255.f,
                     255.f, 0.f,   170.f, 255.f, 0.f,   85.f,  255.f, 0.f,
                     0.f,   255.f, 255.f, 0.f,   170.f, 170.f, 0.f,   255.f,
  ...
  for (int person = 0; person < keyshape[0]; ++person) {
    ...
    for (int pair = 0u; pair < pairs_size; pair += 2) {
      ...
      const int index2 =
        (person * num_keypoints + pairs[pair + 1]) * keyshape[2];
      if (keypoints[index1 + 2] > threshold &&
        ...
        Point keypoint1{ intRound(keypoints[index1] * scale),
                         intRound(keypoints[index1 + 1] * scale) };
        Point keypoint2{ intRound(keypoints[index2] * scale),
    ...
    for (int part = 0; part < num_keypoints; ++part) {
      ...
      if (keypoints[index + 2] > threshold) {
        ...
        Scalar color{ colors[(color_index + 2) % number_colors],
                      colors[(color_index + 1) % number_colors],
                      colors[(color_index + 0) % number_colors] };
        Point center{ intRound(keypoints[index] * scale),
                      intRound(keypoints[index + 1] * scale) };
...
static void
connect_bodyparts(vector<float>& pose_keypoints,
                  ...
                  vector<int>& keypoint_shape)
{
  ...
  const int body_part_pairs[] = { 1,  2,  1,  5,  2, 3,  3, 4,  5,  6,
                                  6,  7,  1,  8,  8, 9,  9, 10, 1,  11,
                                  11, 12, 12, 13, 1, 0,  0, 14, 14, 16,
                                  0,  15, 15, 17, 2, 16, 5, 17 };
  const int limb_idx[] = { 31, 32, 39, 40, 33, 34, 35, 36, 41, 42, 43, 44, 19,
  ...
  for (unsigned int pair_index = 0u; pair_index < num_body_part_pairs;
       ++pair_index) {
    ...
    } else // if (nA != 0 && nB != 0)
    {
        ...
        for (int j = 1; j <= nB; j++) {
          ...
          if (norm_vec > 1e-6) {
            ...
            for (int lm = 0; lm < num_inter; lm++) {
              const int mX =
                fastMin(mapw - 1, intRound(sX + lm * dX / num_inter));
              const int mY =
                fastMin(maph - 1, intRound(sY + lm * dY / num_inter));
      ...
      // assuming that each part occur only once, filter out same part1 to
      // different part2
      ...
      for (unsigned int row = 0u; row < temp.size(); row++) {
        ...
        if (!occurA[aidx - 1] && !occurB[bidx - 1]) {
          ...
          connectionK.emplace_back(
            std::make_tuple(body_partA * peaks_offset + aidx * 3 + 2,
                            body_partB * peaks_offset + bidx * 3 + 2,
                            score));
          ++counter;
          if (counter == minAB) {
            break;
      ...
      // connection initialize first body part connection
      if (pair_index == 0) {
          ...
          for (unsigned int i = 0u; i < connectionK.size(); ++i) {
            ...
            for (unsigned int j = 0u; j < subset.size(); j++) {
              if (subset[j].first[body_partA] == indexA) {
                ...
                ++num;
                subset[j].first[subset_counter_index] =
                  subset[j].first[subset_counter_index] + 1;
  ...
  for (unsigned int index = 0; index < subset.size(); ++index) {
    ...
    if (subset_counter >= min_subset_cnt &&
        (subset_score / subset_counter) > min_subset_score) {
      ++number_people;
  ...
  for (unsigned int person = 0u; person < valid_subset_indexes.size();
       ++person) {
...
static void
find_heatmap_peaks(const float* src,
                   float* dst,
                   const int SRCW,
                   const int SRCH,
                   const int SRC_CH,
                   const float TH)
{
  // find peaks (8-connected neighbor), weights with 7 by 7 area to get
  // sub-pixel location and response
      ...
      for (x = 1; x < SRCW - 1 && num_people != POSE_MAX_PEOPLE; ++x) {
        ...
        if (value > TH) {
          ...
          if (value > TOPLEFT && value > TOP && value > TOPRIGHT &&
              value > LEFT && value > RIGHT && value > BUTTOMLEFT &&
              value > BUTTOM && value > BUTTOMRIGHT) {
            float x_acc = 0;
            float y_acc = 0;
            float score_acc = 0;
            for (i = -3; i <= 3; ++i) {
...
static Mat
create_netsize_im(const Mat& im, const int netw, const int neth, float* scale)
{
  // for tall image
  int newh = neth;
...
void
run_pose(int ac, char** av)
{
  if (ac < 5) {
    cout << "usage: ./darknet pose [demo] <img file> <cfg file> <weight file>"
         << endl;
    return;
  }
  ...
  do {
    ...
    // 8. get heatmap peaks
    float* heatmap_peaks =
      new float[3 * (POSE_MAX_PEOPLE + 1) * (NET_OUT_CHANNELS - 1)];
    find_heatmap_peaks(
      heatmap, heatmap_peaks, net_inw, net_inh, NET_OUT_CHANNELS, 0.05);

    // 9. link parts
    vector<float> keypoints;
    vector<int> shape;
    connect_bodyparts(keypoints,
                      heatmap,
                      heatmap_peaks,
                      net_inw,
                      net_inh,
